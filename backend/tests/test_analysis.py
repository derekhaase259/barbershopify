...
def test_separate_vocal_extracts_melody_from_the_stem(test_wav, monkeypatch):
    # the isolated "vocal" is a steady B natural (pitch class 11): in C major
    # so snap-to-key leaves it, yet the C-major mix melody never sounds it — so
    # its dominance proves the melody was read off the stem, not the mix
    dur = len(MELODY) * BEAT
    t = np.linspace(0, dur, int(SR * dur), endpoint=False)
    stem = (0.5 * np.sin(2 * np.pi * 493.88 * t)).astype(np.float32)
...
def test_separate_vocal_falls_back_to_mix_when_unavailable(test_wav, monkeypatch):
    ...
    result = analyze(str(test_wav), use_cache=False, separate_vocal=True)
    # the C-major mix melody survives the fallback (no B-natural takeover)
    pcs = [n.midi % 12 for n in result.input.melody]
    assert pcs and sum(p == 11 for p in pcs) / len(pcs) < 0.2
