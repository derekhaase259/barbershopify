...
from barbershop.lookup.tabs import TabChords, fetch_candidates, parse_chordpro
...
WILSON_PRO = "{t:Hey Jude}\n{st:Wilson Pickett}\n[Am]every[D]body [E7]now [Am]yeah [F]come [G]on"
...
def test_fetch_candidates_ranks_artist_matches_before_covers(monkeypatch):
    calls = []
    ...
    def fake_get(url, headers=None, timeout=None):
        ...
        if "/wilson/1.html" in url:
            return _Page(_song_html(WILSON_PRO))  # same title, other artist: a cover
        return _Page(_song_html(CHORDPRO))

    monkeypatch.setattr("barbershop.lookup.tabs.requests.get", fake_get)
    candidates = fetch_candidates(BEATLES)
    assert [c.artist for c in candidates] == ["The Beatles", "Wilson Pickett"]
    assert candidates[0].chords == ["C", "G", "G7", "C", "F", "C"]
    # 2 searches + wilson + beatles = 4 requests; dup links were not refetched
    assert len(calls) == 4


def test_fetch_candidates_rejects_wrong_titles(monkeypatch):
    wrong_song = "{t:Jude the Obscene}\n{st:Therapy?}\n[Am]not [D]this [E]song [F]at [G]all"

    def fake_get(url, headers=None, timeout=None):
        if "results.php" in url:
            return _Page('<a href="/chord.pere/x/1.html">x</a>')
        return _Page(_song_html(wrong_song))

    monkeypatch.setattr("barbershop.lookup.tabs.requests.get", fake_get)
    assert fetch_candidates(BEATLES) == []


def test_fetch_candidates_requires_minimum_chords(monkeypatch):
    short = "{t:Hey Jude}\n{st:The Beatles}\n[C]too [G]short"
...
def test_fetch_candidates_never_raises(monkeypatch):
